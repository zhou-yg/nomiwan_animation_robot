...
//爬取分享的flash query器
var queryDir = '../query/';
var queryVideoSrcModule = queryDir + 'queryVideoSrc_episodes';
            ...
            if(currentEpisodesIndex<episodesLen){
                try{
                    ...
                    queryVideoSrcPromise.done(function (videoSrcObj) {
                        var t2 = +new Date();
                        var cost = (t2 - t1) / 1000;
                        utils.log(animationName+' 第'+animationIndex+'个剧集，第'+currentEpisodesIndex+'个源 耗时：', cost, ' 秒 ===============');
                    ...
                    queryVideoSrcPromise.fail(function (err) {
                        utils.log('********'+err+'***********');
...
module.exports = {
    ...
    getVideoSrc:function(option,cb){
        ...
        destAllVideoSrcJson = destAllVideoSrcJson + utils.getEpisodesPiecesName(startIndex,eachQueryEpisodesNum);
